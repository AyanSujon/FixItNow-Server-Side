...
const createPaymentsInDB = async (customerId, payload) => {
    const { bookingId, amount, method, provider, currency = "USD", } = payload;
    // Check booking
...
//     })
// }
const createCheckoutSeassion = async (userId, bookingId) => {
    const transactionResult = await prisma.$transaction(async (tx) => {
        const user = await tx.user.findUniqueOrThrow({
            where: {
                id: userId,
        ...
        const existingPayment = user.payments.find((payment) => payment.stripeCustomerId);
...
const getAllPaymentHistoryFromDB = async (userId) => {
...
const getPaymentDetailsByIdFromDB = async (paymentId) => {
